...
Ejecución:
streamlit run dashboard_deep_2.py
"""
...
# ============================================================================
# NUEVA SECCIÓN: MAPA DE CALOR INTERACTIVO
# ============================================================================
st.header("🌍 MAPA DE CALOR GEOGRÁFICO")
...
fig_mapa = px.scatter_geo(
    ventas_ciudad,
    lat=[-31.4, -31.4, -33.1, -31.6, -32.4, -31.3],  # Coordenadas aproximadas de ciudades
    lon=[-64.2, -64.2, -64.3, -64.4, -63.2, -64.3],
    size='importe',
    color='importe',
    hover_name='ciudad',
    size_max=50,
    title="Mapa de Calor: Ventas por Ciudad",
    color_continuous_scale='Viridis'
)
...
if modo == "🔍 Modo Analista":
    ...
    with subtab4:
        ...
        top_clientes = datos_filtrados.groupby('id_cliente').agg({
            'importe': 'sum',
            'id_venta': 'nunique',
            'cantidad': 'sum'
        }).nlargest(10, 'importe')
...
<div style='text-align: center; padding: 1rem; background-color: #1f77b4; color: white; border-radius: 8px;'>
    <p style='margin: 0; font-size: 1.1rem;'>
        ...
        Análisis Comercial Avanzado | Versión 2.0
    </p>
</div>
""", unsafe_allow_html=True)