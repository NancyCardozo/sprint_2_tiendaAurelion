...
Ejecución:
streamlit run dashboard_completo_ok.py
"""
...
# TAB 6: ANÁLISIS COMPLETO (MARKDOWN EXTERNO)
with tab6:
    st.header("📋 ANÁLISIS COMERCIAL COMPLETO")
    
    # Intentar cargar el archivo markdown desde la raíz del proyecto
    try:
        with open('analisis_comercial.md', 'r', encoding='utf-8') as file:
            contenido_md = file.read()
        st.markdown(contenido_md, unsafe_allow_html=True)
        
    except FileNotFoundError:
        st.error("⚠️ No se encontró el archivo 'analisis_comercial.md'")
        
        # Mostrar información básica como respaldo
        st.markdown("""
        ## 📊 Resumen Ejecutivo - Tienda Aurelion
        
        **Período analizado:** Enero - Junio 2024  
        **Total ventas:** $3,266,246  
        **Transacciones:** 120 ventas | 431 líneas de detalle  
        **Clientes activos:** 67 de 100 registrados (67%)
        
        ---
        
        ### 🔴 Problemas Críticos Detectados
        
        1. **Baja frecuencia de compra** - Solo 0.67 ventas/día
        2. **Productos sin rotación** - 5 productos con 0 ventas
        3. **Categoría Limpieza subdesarrollada** - 24.7% vs 35-40% esperado
        4. **Tasa de conversión baja** - 32% vs 60% objetivo
        5. **Alta volatilidad** - Caída de -37.5% en Abril
        6. **33% clientes inactivos** - Potencial perdido de $900K
        
        ---
        
        ### 💡 Soluciones Propuestas
        
        | Estrategia | Inversión | ROI | Prioridad |
        |------------|-----------|-----|-----------|
        | Fidelización | $185K | 1,157% | ⭐⭐⭐ |
        | Marketing Automation | $150K | 10,343% | ⭐⭐⭐ |
        | Pricing & Promos | $150K | 1,340% | ⭐⭐⭐ |
        ...
        **ROI Total:** 1,349%
        ...
        st.error(f"❌ Error al cargar el archivo markdown: {e}")
        st.markdown("""
        ## Error al cargar el análisis completo
        
        Se produjo un error al intentar cargar el archivo markdown externo.
        Por favor, verifica que el archivo exista y tenga el formato correcto.
        """)
